	...
	jsonp = function(xOptions) {
	    ...
	    later(function() {
	        ...
	        xOptions.abort = cleanUp = function() {
	            ...
	            // Targeting a specific browser version is so bad, but what else can we do?
	            var BrowserNeedsBackButtonHackFix = function() {
	                var majorVersion = parseFloat($.browser.version.substr(0, 3));
	                var buildNumber = parseInt($.browser.version.substr(4, 4));
	                // We need the crappy hacky fix if we're dealing with a firefox browser with a gecko rev #more recent than 1.9.2
	                return $.browser.mozilla && (majorVersion > 1.9 || (majorVersion == 1.9 && buildNumber >= 2));
	            };

	            if (BrowserNeedsBackButtonHackFix) {
	                var splitUrl = location.href.split('#');
	                var url = splitUrl[0];
	                var dateTime = new Date();
	                // this makes sense in my specific use of jsonp.  Really the key here is that we modify the url in a way that will
	                // make it unique from the duplicate entry that firefox deposits in the history.
	                // This specific fix is not a general solution because it leans upon the fact that I'm not using the fragment ID 
	                // part of the url for anything on this page of my site.  Really the key is to ensure the url is different in a way that
	                // causes minimal impact to the functioning/presentation of the affected page.
	                location.replace(url + '#updated_' + dateTime.getHours() + '_' + dateTime.getMinutes() + '_' + dateTime.getSeconds());
	            }
